...
import Util from "../../screens/utility/utility";
...
const Banner = (props) => { 
  ...
  const Slide = function Slide({ data }) {   
    return (  
      (
        props.type == 'semi'?
          <TouchableOpacity key={data.id} activeOpacity={0.91} style={[styles.slide,{backgroundColor:'#ffffff'},data.id == 1?{marginLeft:8,marginRight:10}:{marginLeft:8,marginRight:10}, data.id == data.length?{}:{}]} > 
                {/* <Image 
                  // source={data.image} 
        ...
        (
          props.type == 'full'?
            <TouchableOpacity key={data.id} activeOpacity={0.91} style={[styles.slideFull,{position:'relative'}]}>  
              <FastImage  resizeMode={FastImage.resizeMode.contain}  source={{uri:data.image}} style={[styles.slideImageFull]} />
