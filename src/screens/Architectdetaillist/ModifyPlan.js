...
function ModifyThisPLan({route,navigation}) {
        ...
        useCallback(() => { 
            if(route.params.projectId){
                ...
                AsyncStorage.getItem('UserToken',(error,tokenCreds)=>{
                    setToken(tokenCreds);
                    console.log(`link--: ${GlobalData.LINKURL}modify_plan/${route.params.projectId}/architectural-design?viewmode=app&key=${tokenCreds}`);
                }); 
            }else{
               // setLoader(false);
            }
        ...
        <View style={{flex:1}}>
            {
                ...
                {/* <Text>{`https://www.makemyhouse.com/modify_plan/${route.params.projectId}/architectural-design?token=${token}`}</Text> */}
                <View style={{flex:1}}>
                    ...
                    cacheEnabled={false} 
                    source={{ uri: `${GlobalData.LINKURL}modify_plan/${route.params.projectId}/architectural-design?viewmode=app&key=${token}` }}  style={{ width: '100%', height: '100%' }}  />
                </View>
