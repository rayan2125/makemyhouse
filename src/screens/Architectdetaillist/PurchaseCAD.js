...
function PurchaseCAD({route,navigation}) {
    ...
    useCallback(() => { 
        if(route.params.projectId){
            ...
            AsyncStorage.getItem('UserToken',(error,tokenCreds)=>{
                setToken(tokenCreds);
                console.log(`${GlobalData.LINKURL}cad-file/${route.params.projectId}/architectural-design?viewmode=app&key=${tokenCreds}`); 
            }); 
        }else{
           // setLoader(false);
        }
    ...
    return (
        <View style={{flex:1}}>
            ...
            <View style={{flex:1}}>
                    ...
                    {/* <Text>{`https://www.makemyhouse.com/cad-file/${route.params.projectId}/architectural-design?token=${token}`}</Text> */}
                    <WebView  
                    ...
                    cacheEnabled={false}  source={{ uri: `${GlobalData.LINKURL}cad-file/${route.params.projectId}/architectural-design?viewmode=app&key=${token}`}}  style={{ width: '100%', height: '100%' }}  />
            </View>        
