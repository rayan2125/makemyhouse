...
//main screen 
const DetectLocation = ({ route }) => {

    const navigation = useNavigation();
    console.log("detech Location: ", route)
    const fromPage = route.params != undefined ? route.params.fromPage : 'accountCreate';

    useEffect(() => {
        const knowTheNavigation = () => {
            if (fromPage == 'accountCreate') {
    ...
    return (
        <ScreenWrapper>
            ...
            <ScndHeader
                ...
                Search={false}
                Profile={false}
                Back={fromPage == "package" ? true : fromPage == 'home' ? true : false}
                BackScreen=""
                Skip={false}
                SkipScreen=""
            />
